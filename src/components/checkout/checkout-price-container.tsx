...
export function CheckoutPriceContainer({ checkoutData }: Props) {
  ...
  return (
    <>
      <div className={'text-base leading-[20px] font-semibold'}>অর্ডারের সারাংশ</div>
      ...
      {recurringTotal !== undefined ? (
        billingCycle && (
          <div className={'pt-4 text-base leading-[20px] font-medium text-muted-foreground'}>
            তারপর {formatMoney(recurringTotal, checkoutData?.currency_code)} {formatBillingCycle(billingCycle)}
          </div>
        )
      ) : (
